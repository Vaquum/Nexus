...
- Fix [`configure_logging`](nexus/infrastructure/observability.py)'s `ProcessorFormatter.foreign_pre_chain` to include `structlog.stdlib.ExtraAdder()` so stdlib-logger `extra={...}` payloads merge into the structlog event dict before `JSONRenderer` serializes it. Pre-patch every `_log.info('msg', extra={'strategy_id': X, ...})` call from a stdlib logger silently dropped its extras — only `event` / `level` / `timestamp` made it to JSON. This affected every Nexus emit site that uses `_log = logging.getLogger(__name__)`: [`action_submit.py`](nexus/strategy/action_submit.py)'s 7 emit sites (`build_context raised`, `context unavailable, skipping action`, `validator raised`, `action rejected by validator`, `translate_to_trade_command raised`, `send_command failed`, `action submitted`), [`outcome_processor.py`](nexus/infrastructure/praxis_connector/outcome_processor.py), [`praxis_outbound.py`](nexus/infrastructure/praxis_connector/praxis_outbound.py), [`capital_controller.py`](nexus/core/capital_controller/capital_controller.py), and the validator stages — every per-action diagnostic field (`strategy_id`, `action_type`, `failed_stage`, `reason_code`, `command_id`) was on the floor in the JSON sink. Observed in production after the v0.59.2 Praxis deploy: `{"event": "action rejected by validator", "level": "warning", "timestamp": "..."}` lines with no `strategy_id`, no `failed_stage`, no `reason_code` — root-causing `StubStrategySFD_4permut` validator rejections required guessing against grep over the container log. The native structlog API (`structlog.get_logger(...)`, `_log.info('msg', strategy_id=X)`) bypasses the stdlib bridge entirely and was unaffected — but only `sequencer.py` / `shutdown_sequencer.py` use that API; every emit site in the trade lifecycle uses stdlib logging. The Praxis-side mirror lands in [Vaquum/Praxis#105](https://github.com/Vaquum/Praxis/pull/105) (v0.59.3) — both observability modules need the same fix because the launcher and Nexus both contribute log lines to the same JSON sink

### Add

- Add `bound_context(**kwargs)` context manager to [`nexus/infrastructure/observability.py`](nexus/infrastructure/observability.py) wrapping `structlog.contextvars.bound_contextvars` so callers do not need to import `structlog` directly. Bind keys for the lifetime of a `with`-block; reset on exit (including the exception path) so the caller's outer context is restored without manual `clear_context()` calls. Exported via `__all__`
- Wire [`submit_actions`](nexus/strategy/action_submit.py) per-iteration body in `with bound_context(strategy_id=..., action_type=..., trade_id=..., command_id=...)`. The bind kwargs include `strategy_id` (always), `action.action_type.value` (always), `action.trade_id` (when present — EXIT and MODIFY actions), and `action.command_id` (when present — ABORT actions and pre-bound MODIFY/EXIT references). Any downstream emit (validator stages, capital_controller, praxis_outbound, OutcomeProcessor) inside the iteration body now carries the correlation fields via structlog's contextvars merge_contextvars processor — without each emit site having to thread the fields through `extra={...}`. Per-iteration scope: keys bound on entry are reset on exit, so a long-running outer caller's structlog context (`account_id` etc.) is not polluted across actions

### Tests

- Add `test_stdlib_extras_appear_in_json` regression test in [`tests/test_observability.py`](tests/test_observability.py) pinning the new `ExtraAdder` contract: emit a stdlib log with `extra={'strategy_id': 'stub_always_one', 'action_type': 'enter', 'failed_stage': 'capital', 'reason_code': 'CAPITAL_BUDGET_EXCEEDED'}`, capture stdout JSON, assert the four extras appear as top-level fields. Verified locally that removing `ExtraAdder` from `foreign_pre_chain` deterministically fails the test with `KeyError: 'strategy_id'`
- Add `test_bound_context_binds_and_unbinds` and `test_bound_context_resets_on_exception` in [`tests/test_observability.py`](tests/test_observability.py) pinning the `bound_context` lifecycle. The first nests an inner bind inside an outer `bind_context(account_id=...)` and asserts both bindings appear inside the with-block while only the outer survives after exit. The second raises an exception inside the with-block and asserts the bound key is still unbound on the outside (verifies the `__exit__` path reset)
- Add `TestPerActionBoundContext` (4 cases) in [`tests/test_action_submit.py`](tests/test_action_submit.py) pinning per-action bind: (1) the validator's `validate(ctx)` runs inside the per-action with-block — captures `structlog.contextvars.get_contextvars()` at validate time and asserts `strategy_id` + `action_type` are bound; (2) per-action keys do not leak past the `submit_actions` return — captures contextvars after the call and asserts none of `strategy_id` / `action_type` / `trade_id` / `command_id` survived; (3) an EXIT action's `trade_id` is bound for the iteration; (4) an ABORT action's `command_id` is bound (captured inside the `praxis_outbound.send_abort` shim since ABORTs bypass the validator path)
