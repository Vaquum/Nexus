...
- Add `_values_for_log` helper that summarizes sequence values longer than 16 elements as `<sequence type=X len=N>` so a future predictor that returns a vector (current predictors collapse to scalars in `signal_producer._extract_values`) cannot blow up the structured log line
- Pin both behaviors in [`tests/test_predict_loop_tick_once.py`](tests/test_predict_loop_tick_once.py): `test_logs_signal_before_dispatch` asserts the log record carries the expected extras; `test_values_for_log_passes_scalars_through` + `test_values_for_log_truncates_long_sequences` cover the helper's scalar pass-through and sequence-truncation branches
