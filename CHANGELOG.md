...
- Add [`RiskState.lock`](nexus/core/domain/risk_state.py) transient attribute and [`RiskState.lock_cm`](nexus/core/domain/risk_state.py) helper that returns the lock as a context manager (or `nullcontext()` when unset) so the OutcomeProcessor writer, HealthLoop refresher, and validator reader coordinate on the same lock instance (FINAL-MAJOR-02)
...
- Add 5 pre-PR review TD entries (TD-067..TD-071) to [`docs/TechnicalDebt.md`](docs/TechnicalDebt.md) covering the dead-code helper, dataclass-bypass attribute, empty-string sentinel, decoder duplication, and unlocked recover writes
...
- Fix `order_fill` strict fee-deficit check tripping on sub-ULP precision drift between `proportional_estimated` (computed via subtraction round-trip) and `actual_fees` (computed via direct division). On awkward notional/fee ratios (e.g. notional=7, fee=1) the deficit accumulated and silently rejected fills with `EXPECTED_MISS` after a few partials. Post-fix uses `_SUB_ULP_TOLERANCE` on the deficit check; real deficits (>1E-12 quote) still hard-fail (FINAL-MAJOR-09)
