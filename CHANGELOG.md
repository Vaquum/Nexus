...
- Wire EXIT FILL through `order_exit` in [`OutcomeProcessor._handle_fill`](nexus/infrastructure/praxis_connector/outcome_processor.py) — when `not context.is_entry`, after `_reduce_position` succeeds, decrement capital aggregates by `position.avg_cost_basis * fill_size`. Closes the round-trip conservation gap that locked accounts after ~7 round-trips at 80% utilization (or ~1.5 round-trips per strategy at the per-strategy budget cap) — eleven audit rounds missed this because EXIT-FILL tests asserted `success` and `realized_pnl` but never `state.capital.position_notional` or `per_strategy_deployed` (BLOCKER-A.3)
