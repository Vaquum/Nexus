...
- Add operational-mode enforcement in [`validate_intake_stage`](nexus/core/validator/intake_stage.py); ENTER actions are rejected with reason `INTAKE_MODE_BLOCKS_ENTER` when `state.mode.mode != OperationalMode.ACTIVE`, and EXIT actions are rejected with `INTAKE_MODE_HALTED_BLOCKS_EXIT` when mode is HALTED. CANCEL/ABORT remain available in HALTED so an operator can wind pending orders down without flipping the mode back manually. Closes the gap where `HealthLoop` flipped `state.mode` to REDUCE_ONLY/HALTED on health degradation but no validator stage actually enforced the documented contract (PT-FIX-15)
...
- Add [`WriteAheadLog.read_safe()`](nexus/infrastructure/wal.py) that bounds reads at `_find_valid_end()` and silently stops at a torn tail. Switch [`StateStore.__init__`](nexus/infrastructure/state_store.py), `StateStore.recover`, and `StateStore.read_events` to consume the safe variant so a process killed mid-`append` no longer blocks the next boot. `read_all` retains strict raise-on-corruption semantics for callers that want it. The torn record itself is unrecoverable and stays discarded; the file is cleaned up by the next `append()` via existing self-truncation (PT-FIX-17)
