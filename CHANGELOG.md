...
- Add `TestInjectReferencePrice` test class in [`tests/test_signal_producer.py`](tests/test_signal_producer.py) with 5 cases: `close` injected when absent, predictor-supplied `close` preserved verbatim, empty `market_data` is a silent no-op, missing `close` column is a silent no-op, non-numeric `close` value is a silent no-op
