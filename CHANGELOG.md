...
## v0.49.0 on 21st of May, 2026

- Add `_inject_reference_price` shim in [`nexus/strategy/signal_producer.py`](nexus/strategy/signal_producer.py) that, after `predict()` returns, forwards the latest raw `close` from the pre-feature-engineering `market_data` frame into the `Signal.values` dict when the predictor didn't include `close` itself. ML pipelines that run `close` through `fractional_diff` (or any other column-replacing transform in `prepare_data`) cannot recover the raw price from inside `predict()`; rule-based / stub predictors that forward `close` directly are unaffected (existing values are never overwritten). Closes the silent-no-op failure mode where strategies keying on `signal.get('close')` for ENTER notional sizing produced zero commands despite firing `_preds=1` predictions every tick
- Add `TestInjectReferencePrice` test class in [`tests/test_signal_producer.py`](tests/test_signal_producer.py) with 10 cases: `close` injected when absent, predictor-supplied `close` preserved verbatim, empty `market_data` is a silent no-op, missing `close` column is a silent no-op, non-numeric `close` value is a silent no-op, NaN `close` rejected at the boundary, +Infinity rejected, -Infinity rejected, Python `bool` close rejected, `numpy.bool_` close rejected
- Reject non-finite `close` (NaN / +/-Infinity) at the injection boundary in `_inject_reference_price`; `float(polars_nan)` returns `math.nan` without raising, so without the `math.isfinite()` guard the non-finite value would reach `Signal.__post_init__` (`nexus/strategy/signal.py:45-46`) which raises `ValueError: values['close'] must be finite` and crashes the predict-loop tick. The boundary guard converts the crash into a silent skip consistent with the shim's fail-silent contract on every other guard path. Mirrors the Nexus-wide validation_finiteness pattern of rejecting at the boundary
- Reject `bool` and `numpy.bool_` `close` values before the float cast in `_inject_reference_price`; `float(True) == 1.0` / `float(False) == 0.0` are both finite and would slip past `math.isfinite`, injecting a bogus 0/1 reference price. Matches the canonical Nexus bool-exclusion pattern (`isinstance(val, bool)` — see `health_evaluator.py:42`, `:52`, `:105`, `:111`). Polars scalar access can return either Python `bool` or `np.bool_` depending on dtype, so both are checked explicitly (np.bool_ is not a `bool` subclass)

## v0.50.0 on 23rd of May, 2026

### Update

- Bump `vaquum_limen` git+https pin from `v3.0.6` to `v3.9.0` ([Vaquum/Limen@v3.9.0](https://github.com/Vaquum/Limen/releases/tag/v3.9.0)). Limen v3.9.0 ships the new YAML-based SFD pipeline (`limen/yaml/` package — `parser`, `compiler`, `resolver`, `validator`, `rules`) that unblocks experiment_runner bundles declaring `metadata.json["sfd_module"] = "yaml:<name>"` with a `manifest.yml` instead of a `<sfd_module>.py` predictor file. The Limen `Trainer` API surface that Nexus depends on is unchanged across the v3.0.6 → v3.9.0 jump: `Trainer.__init__(experiment_dir, data=None)`, `Trainer.train(permutation_ids: list[int])`, the private `_data` / `_manifest` attribute reads in [`nexus/startup/sequencer.py`](nexus/startup/sequencer.py) (lines 634, 650-651 — flagged by `# NOTE` comments) all still exist; `RuleBasedManifest` is still re-exported from `limen.experiment` ([`limen/experiment/__init__.py`](https://github.com/Vaquum/Limen/blob/v3.9.0/limen/experiment/__init__.py)) and still extends the same `Manifest` base with `with_params_override(...)` / `prepare_data(...)` shape consumed by [`produce_signal`](nexus/strategy/signal_producer.py). 1385 Nexus tests pass and 17 are skipped against Limen v3.9.0 unmodified — including the 10 `test_limen_trainer_contract.py` cases that pin the `Trainer` public contract and the rule-based / ML-manifest branch tests in `test_signal_producer.py`; the one `PytestUnhandledThreadExceptionWarning` in `test_outcome_processor.py::TestFinalMajor08RealizedPnlIsNetOfFees::test_winning_exit_realized_pnl_is_net_of_fees` (worker thread `FileNotFoundError` on WAL teardown) is pre-existing test-cleanup race noise unrelated to the Limen pin. Nexus production code is not modified by this bump
- Update `vaquum_limen 3.0.6` → `vaquum_limen 3.9.0` in two [`nexus/startup/sequencer.py`](nexus/startup/sequencer.py) comments (lines 631, 651) flagging the private-attribute reads (`cached_trainer._data`, `trainer._manifest`); both private attributes still exist on the Limen v3.9.0 `Trainer` class so the re-validate-on-upgrade contract is satisfied (mirrors the precedent set in [v0.45.0](#v0450-on-10th-of-may-2026) when the prior 3.0.5 → 3.0.6 bump performed the same comment refresh)
- Add transitive runtime dependency on `ruamel.yaml>=0.18` via Limen v3.9.0 (`limen/yaml/parser.py:4` imports `from ruamel.yaml import YAML`). Picked up automatically through Limen's `pyproject.toml` dependency list — Nexus does not import `ruamel.yaml` directly, but loading any `yaml:`-prefixed SFD bundle triggers the import chain through `limen.yaml`. Other Limen v3.9 deps (`scikit-learn>=1.6.1`, `numpy>=2.2.6`, `pandas>=2.3.1`, `xgboost`, `lightgbm`, `tslearn`, `pyarrow`, `ta-lib`, `click>=8.0`, `scipy`, `statsmodels`) are training-only and already in the broader Vaquum environment via `binancial` and Praxis's `dev` extras — no Nexus runtime addition beyond `ruamel.yaml`

## v0.51.0 on 24th of May, 2026

### Update

- Bump `vaquum_limen` git+https pin from `v3.9.0` to `v4.0.1` ([Vaquum/Limen@v4.0.1](https://github.com/Vaquum/Limen/releases/tag/v4.0.1)). v0.50.0 pinned `v3.9.0`, but that tag's `Trainer` cannot reconstruct a YAML-based SFD bundle: `Trainer.__init__` passes `metadata.json["sfd_module"]` (which for a YAML SFD is the `CompiledSFD.__name__` literal `"yaml:<name>"`) straight to `_load_sfd_module`, where the `seg.isidentifier()` path-traversal guard rejects the `:` and raises `ValueError: sfd_module name 'yaml:...' is not a dotted sequence of valid Python identifiers`. The YAML-reconstruction branch — `Trainer.__init__` detecting `metadata.json["yaml_reference"]` and rebuilding the SFD via `CompiledSFD(yaml_reference)` instead of `_load_sfd_module` — only landed in [Limen v3.9.2](https://github.com/Vaquum/Limen/releases/tag/v3.9.2) (commits `7775f9a` / `f37eb74` / `e6ce7eb`, PRs Vaquum/Limen#537-538), tagged after v0.50.0's pin was chosen. Verified locally: a real experiment_runner bundle (`btc_logreg_15m_up_early`, `limen_version: "3.9.2"`, 500 rounds) raises the identifier `ValueError` under v3.9.0 and loads cleanly under v4.0.1 (`Trainer` init OK, `MLManifest` reconstructed, round 24 with 34 populated params)
- Go to `v4.0.1` (latest) rather than the minimal `v3.9.2` fix. The only breaking change in the `v3.9.2 → v4.0.1` span is `UniversalExperimentLoop.run()` defaulting to skip terminal post-processing (Limen v4.0.0) — that is the **training loop**, which Nexus never calls; Nexus only consumes already-trained bundles through `Trainer`. The consumption API surface Nexus depends on is signature-identical across `v3.9.2 → v4.0.1` (`git diff` on `trainer.py` / `historical_data.py` / `manifest_core.py` / `experiment/__init__.py` shows no signature changes to `Trainer.__init__` / `train` / `RuleBasedManifest` / `with_params_override` / `prepare_data` — function bodies may differ where v4.0.x touched UEL internals, but the surface Nexus calls is unchanged), and the YAML schema `VERSION` is still `'1.0'`, so a v3.9.2-produced bundle loads under v4.0.1. 1385 Nexus tests pass and 17 are skipped against Limen v4.0.1 unmodified — including the 10 `test_limen_trainer_contract.py` cases that pin the `Trainer` public contract; the lone `PytestUnhandledThreadExceptionWarning` in `test_outcome_processor.py` (WAL-teardown race) is the same pre-existing test-cleanup noise unrelated to the pin. Nexus production code is not modified by this bump
- Update `vaquum_limen 3.9.0` → `vaquum_limen 4.0.1` in two [`nexus/startup/sequencer.py`](nexus/startup/sequencer.py) comments (lines 631, 651) flagging the private-attribute reads (`cached_trainer._data`, `trainer._manifest`); both private attributes still exist on the Limen v4.0.1 `Trainer` class so the re-validate-on-upgrade contract is satisfied (mirrors the comment-refresh precedent from v0.45.0 / v0.50.0)
