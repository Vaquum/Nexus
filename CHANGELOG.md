...
- Add `TestInjectReferencePrice` test class in [`tests/test_signal_producer.py`](tests/test_signal_producer.py) with 8 cases: `close` injected when absent, predictor-supplied `close` preserved verbatim, empty `market_data` is a silent no-op, missing `close` column is a silent no-op, non-numeric `close` value is a silent no-op, NaN `close` rejected at the boundary, +Infinity rejected, -Infinity rejected
- Reject non-finite `close` (NaN / +/-Infinity) at the injection boundary in `_inject_reference_price`; `float(polars_nan)` returns `math.nan` without raising, so without the `math.isfinite()` guard a non-finite price would land in `Signal.values['close']` and propagate to strategy ENTER sizing as `notional / nan = nan`, silently no-op'ing the order with no log trace. Mirrors the Nexus-wide validation_finiteness pattern of rejecting non-finite values at boundaries rather than relying on every downstream consumer's check
